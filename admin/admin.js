...
document.getElementById('cancel-add-portfolio').addEventListener('click', () => {
...
async function loadPortfolio() {
  ...
  if (!data || data.length === 0) {
    list.innerHTML = '<p class="status">No portfolio items yet. Click "+ Add Item" to get started.</p>';
    return;
  }
...
addPortfolioForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // 2. If user uploaded a file manually, upload it
    ...
    if (thumbnailUrl) {
      ...
      portfolioStatus.className = 'status';
      portfolioStatus.textContent = '✓ Item added. No thumbnail — use the 📷 Thumb button to upload one.';
    }
...
document.getElementById('portfolio-list').addEventListener('click', async (e) => {
  ...
  if (action === 'refresh-thumb') {
    ...
    if (thumbUrl) {
      ...
      alert('Auto-fetch failed. Use the 📷 Thumb button to upload an image manually.');
...
async function tryAutoThumbnail(itemId, videoUrl) {
  try {
    ...
    // Download the image, upload to Supabase Storage so it never expires
...
async function uploadThumbnailBlob(itemId, blob) {
  ...
  // Bust the cache so the browser loads the fresh image
...
document.getElementById('content-list').addEventListener('click', async (e) => {
  const btn = e.target.closest('button');
  if (btn && btn.dataset.action === 'delete-content') {
    if (!confirm('Delete this card?')) return;
    await supabase.from('content_cards').delete().eq('id', btn.dataset.id);
    loadContent();
  }
});
...
async function loadSettings() {
  ...
  (data || []).forEach((s) => {
    ...
    row.innerHTML = `
      <label>${escapeHtml(s.key.replace(/_/g, ' '))}</label>
      <textarea data-key="${s.key}">${escapeHtml(s.value || '')}</textarea>
      <button data-action="save-setting" data-key="${s.key}">Save</button>
    `;
...
document.getElementById('settings-list').addEventListener('click', async (e) => {
  const btn = e.target.closest('button');
  if (!btn || btn.dataset.action !== 'save-setting') return;
